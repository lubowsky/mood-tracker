...
export async function daytimeConversation(
  ...
  if (data === "daytime_hard") {
    ...
    if (choice === "go_main") {
      console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!', ctx.hasAccess)
      // await ctx.reply("🏠 Главное меню:", { reply_markup: getMainMenu(true) })
      await ctx.reply("🏠 Главное меню:", { reply_markup: getMainMenu(!!hasAccess) })
      return
    }
  ...
  "Ты также можешь воспользоваться кнопкой «📝 Добавить запись» в главном меню, а потом обсудить это с психологом.",
    ...
    if (choice === "hard_no") {
      await next.editMessageText(
        "Сочувствую тебе…\n" +
  "Пожалуйста, не оставайся один(одна) в своих переживаниях.\n\n" +
  "Можно позвонить или написать другу — переживать вдвоём действительно легче.\n" +
  "Ты также можешь воспользоваться кнопкой «📝 Добавить запись» в главном меню, а потом обсудить это с психологом.",
        {
          reply_markup: {
  ...
  async function handleHardFinal(query: MyContext, ctx: MyContext) {
    ...
    if (data === "hard_help_friend") {
      await query.editMessageText(
        ...
        "Подумай, кому ты мог(могла) бы написать или позвонить прямо сейчас 💛"
      )
    }
