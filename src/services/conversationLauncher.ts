 ...
 *
 * Behaviour:
 * 1) Try to clear stored session for the user via available session storage helpers
 *    (bot.sessionStorage.write or bot.session.write) — this prevents stuck conversations.
 * 2) If session storage isn't accessible, send a soft /cancel fake-update as a fallback.
 * 3) Send the actual fake update `/launch_<name>` to start the conversation.
 */
export async function launchConversation(
  bot: Bot<MyContext>,
  name: string,
  ...
  try {
    ...
    try {
      // `bot as any` because grammy internal session/storage types can vary by middleware
      const maybeSessStor = (bot as any).sessionStorage
      const maybeSess = (bot as any).session

      if (maybeSessStor && typeof maybeSessStor.write === "function") {
        // Write an empty session object for this user id (clears conversations)
        await maybeSessStor.write(String(telegramId), {})
        console.log(`🧹 Cleared session storage for ${telegramId} (via bot.sessionStorage.write)`)
      } else if (maybeSess && typeof maybeSess.write === "function") {
        // Some setups expose `session` with read/write
        await maybeSess.write(String(telegramId), {})
        console.log(`🧹 Cleared session storage for ${telegramId} (via bot.session.write)`)
      } else {
        console.log(`ℹ️ No accessible sessionStorage on bot instance — will try soft cancel before launch`)
      }
