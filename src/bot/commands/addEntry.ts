...
// import { timeOfDayKeyboard, intensityKeyboard, cancelKeyboard, mainMenu } from '../keyboards';
...
// Оцени общее физическое состояние по шкале от 1 до 10:
...
//         const physicalIntensity = parseInt(text);
//         if (!validateIntensity(physicalIntensity)) {
//           await ctx.reply('Пожалуйста, введите число от 1 до 10');
//           return;
//         }
        
//         session.data.overallPhysical = physicalIntensity;
//         session.step = 'emotions';
        
//         await ctx.reply(`💭 *Эмоции и чувства*

// Опиши свои эмоции (через запятую):
// • Тревога
// • Радость  
// • Гнев
// • Спокойствие
...
// *Пример:* "тревога, раздражение, усталость"`, {
//           parse_mode: 'Markdown',
//           reply_markup: cancelKeyboard
//         });
//         break;
...
//       case 'emotions':
//         if (text.toLowerCase() !== 'нет' && text.trim() !== '') {
//           const emotions = text.split(',').map(e => e.trim()).filter(e => e);
...
// Оцени общее ментальное состояние по шкале от 1 до 10:
// 1 - Очень плохо, 10 - Отлично`, {
//           parse_mode: 'Markdown',
//           reply_markup: intensityKeyboard
//         });
...
//       case 'mental_intensity':
//         const mentalIntensity = parseInt(text);
//         if (!validateIntensity(mentalIntensity)) {
//           await ctx.reply('Пожалуйста, введите число от 1 до 10');
//           return;
//         }
        
//         session.data.overallMental = mentalIntensity;
//         session.step = 'thoughts';
        
//         await ctx.reply(`💫 *Мысли в моменте*
...
//         await ctx.reply(`📝 *Дополнительная информация* (необязательно)
...
// *Пример:* "Спал 6 часов, стресс 8/10, был сложный разговор"`, {
//           parse_mode: 'Markdown',
//           reply_markup: cancelKeyboard
//         });
//         break;
...
//       case 'additional':
//         // Простой парсинг дополнительной информации
//         if (text.toLowerCase() !== 'нет' && text.trim() !== '') {
//           const lowerText = text.toLowerCase();
          
//           // Парсим сон
//           const sleepMatch = text.match(/(\d+)\s*час/);
//           if (sleepMatch) {
//             session.data.sleepHours = parseInt(sleepMatch[1]);
//           }
          
//           // Парсим стресс
//           const stressMatch = text.match(/стресс\s*(\d+)/i);
//           if (stressMatch) {
//             session.data.stressLevel = parseInt(stressMatch[1]);
//           }
...
//         // Сохраняем запись
//         const entryId = await EntryService.createManualEntry({
//           userId: ctx.user!._id!,
//           ...session.data
//         });
...
// Вот сводка:
// • Физическое состояние: ${session.data.overallPhysical}/10
// • Ментальное состояние: ${session.data.overallMental}/10
// • Симптомы: ${session.data.physicalSymptoms.map(s => s.name).join(', ') || 'нет'}
// • Эмоции: ${session.data.emotions.map(e => e.name).join(', ') || 'нет'}

// Запись #${entryId.toString().slice(-6)} сохранена в ${formatDate(new Date())}. 

// Через некоторое время сможешь проанализировать закономерности в статистике!`, {
//           parse_mode: 'Markdown',
