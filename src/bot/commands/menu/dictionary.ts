...
composer.hears('📚 Справочник эмоций', async (ctx) => {
    const keyboard = new InlineKeyboard()
        .text('Радость', 'emotion_joy').text('Интерес', 'emotion_interest').row()
        .text('Грусть', 'emotion_sadness').text('Страх', 'emotion_fear').row()
        .text('📥 Скачать PDF', 'download_emotions_guide');

    await ctx.reply(`*📚 Справочник эмоций*\nВыберите категорию:`, { 
        parse_mode: 'Markdown', 
        reply_markup: keyboard 
    });
});
